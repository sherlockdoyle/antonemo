...
const antonyms: Record<string, string> = JSON.parse(
  fs.readFileSync("./src/assets/antonyms.json", "utf-8")
);
...
eaclFiles.forEach((file) => {
  const eaclLines = fs
    .readFileSync(path.join("./src/assets/eacl2017", file), "utf-8")
    .trim()
    .split("\n");
...
Object.entries(allWords).forEach(
  ([word, { synonyms, antonyms, _synonyms = [], _antonyms = [] }]) => {
    [word, ...synonyms, ...antonyms, ..._synonyms, ..._antonyms].forEach(
      (w) => {
...
const mainWordFrequencies = Object.fromEntries(
  Object.keys(allWords).map((word) => [word, wordFrequencies[word]])
);
...
topWords.forEach((word) => {
  ...
  if (antonyms.size < 2) finalAntonyms[word] = Array.from(antonyms);
    ...
    finalAntonyms[word] = [
      sortedAntonyms[0],
      sortedAntonyms[sortedAntonyms.length - 1],
    ];
  }
});
...
fs.writeFileSync(
  ...
  JSON.stringify(finalAntonyms, null, 1)
);
